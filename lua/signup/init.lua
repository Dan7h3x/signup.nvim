...
function SignatureHelp.new()
    local self = setmetatable({
        ...
        virtual_text_ns = vim.api.nvim_create_namespace("SignatureHelpVirtualText"),
        last_context = nil,
        visible = false,
...
function SignatureHelp:get_window_opts(width, height)
    ...
    local col = screen_pos.col - 1

    -- Adjust position if completion menu is visible
    if has_pum then
        if pum_pos.row < cursor_pos[1] then
        ...
        -- Default positioning
        if self.config.position.prefer_above then
            row = row - height - 2
        else
            row = row + 1
        end
...
function SignatureHelp:format_signature_line(signature, idx, show_index)
	...
	if signature.parameters and self.config.ui.show_header then
		for i, param in ipairs(signature.parameters) do
			local param_line = string.format(
				"  %s%s: %s",
				self.config.icons.parameter,
				param.label,
				param.documentation
						and (type(param.documentation) == "string" and param.documentation or param.documentation.value)
					or ""
			)
...
function SignatureHelp:display(result)
    ...
    -- Show parameter preview if enabled
    if self.config.features.preview_window then
        self:show_parameter_preview()
    end

    -- Update virtual text if enabled
    if self.config.features.virtual_text then
...
function SignatureHelp:trigger()
    ...
    -- Don't show if completion menu is visible and config says to avoid overlap
    if self.config.behavior.avoid_cmp_overlap and utils.is_completion_visible() then
        self:hide()
        return
    end
...
-- Return module with metatable for setup
